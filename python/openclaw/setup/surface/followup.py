...
def fail(message: str, code: int = 2) -> NoReturn:
    sys.stderr.write(f'[setup_followup_surface][FAIL] {message}\n')
    raise SystemExit(code)


def load_config() -> dict[str, Any]:
    payload = read_repo_contract_json('governance.setup_followups')
    if not isinstance(payload, dict):
        fail(f'{repo_contract_relpath("governance.setup_followups")} 顶层必须为对象')
    return payload
...
def usage() -> str:
    return usage_block(
        canonical_cli_command('setup', 'surface', 'followups', 'show-index'),
        canonical_cli_command('setup', 'surface', 'followups', 'show-entry') + ' --entry <id>',
        canonical_cli_command('setup', 'surface', 'followups', 'show-next-steps') + ' --entry <id> --scenario <scenario_id>',
        canonical_cli_command('setup', 'surface', 'followups', 'show-next-steps') + ' --entry <id> --scenario <scenario_id> --format json',
        title='用法：',
    )


def render_index() -> str:
    lines = [str(load_config().get('title') or 'setup 阶段后续动作参考').strip(), '']
...
def render_next_steps(entry_id: str, scenario_id: str) -> str:
    ...
    lines = []
    for item in list_str(scenario, 'commands'):
        lines.append(item)
    return '\n'.join(lines)
